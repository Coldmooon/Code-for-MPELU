...
__global__ void M2PELUForward(const int n, const int channels, const int dim,
    const Dtype* in, Dtype* out, const int div_factor, const Dtype* alpha, const Dtype* beta) {
  CUDA_KERNEL_LOOP(index, n) {
    int c = (index / dim) % channels / div_factor;
    out[index] = in[index] > 0 ? in[index] : ( exp(beta[c]*in[index] ) - 1 )* alpha[c];
  }
}
...
template <typename Dtype>
__global__ void M2PELUParamBackward(const int n, 
    const int rows, const int rowPitch, const Dtype* in_diff,
    const Dtype* in_data, Dtype* out_diff_alpha, Dtype* out_diff_beta,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    out_diff[index] = in_diff[index] * ((in_data[index] > 0) + (in_data[index] <= 0) * beta[c] * ( top_data[index] + alpha[c] ) );
    for ( int k = 1; k < rows; k++ ) {
      ...
      out_diff[tmp_index] = in_diff[tmp_index] * ((in_data[tmp_index] > 0) + (in_data[tmp_index] <= 0) * beta[c] * ( top_data[tmp_index] + alpha[c] ) );
    }
  }
}

template <typename Dtype>
void M2PELULayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  M2PELUForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
      count, channels, dim, bottom_data, top_data, div_factor, alpha, beta);
  CUDA_POST_KERNEL_CHECK;
}

template <typename Dtype>
void M2PELULayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
//  if (this->param_propagate_down_[0]) {
  if (1) {
    ...
    // NOLINT_NEXT_LINE(whitespace/operators)
    M2PELUParamBackward<Dtype><<<CAFFE_GET_BLOCKS(cdim),
      CAFFE_CUDA_NUM_THREADS>>>(
      cdim, bottom[0]->num(), top[0]->offset(1), top_diff ,
      bottom_data ,
      backward_buff_alpha.mutable_gpu_diff(), backward_buff_beta.mutable_gpu_diff(),
      channels, dim, div_factor, top_data, bottom_diff, alpha, beta);
    CUDA_POST_KERNEL_CHECK;
    if (channel_shared_) {
      Dtype dsum_alpha;
      Dtype dsum_beta;
...
INSTANTIATE_LAYER_GPU_FUNCS(M2PELULayer);
