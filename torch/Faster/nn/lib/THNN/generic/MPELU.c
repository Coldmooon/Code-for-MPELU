          ...
          THIndex_t nOutputPlane)
{
  ...
  else
  {
    ...
    const real *output_data = THTensor_(data)(output);
...
#pragma omp parallel for private(j,k)
    for (i = 0; i < bs; ++i)
    {
      ...
      const real *n_output_data = output_data + i*nOutputPlane*ks;
      ...
      for (j = 0; j < nOutputPlane; ++j)
      {
        ...
        for (k = 0; k < ks; ++k)
        {
          if (n_input_data[k] > 0)
            ...
            n_gradInput_data[k] = n_gradOutput_data[k] * b * (n_output_data[k] + w);
        }
        n_input_data += ks;
        n_output_data += ks;
        n_gradInput_data += ks;
