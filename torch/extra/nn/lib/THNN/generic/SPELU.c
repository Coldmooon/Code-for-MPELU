...
#define TH_GENERIC_FILE "generic/SPELU.c"
#else
...
#pragma omp parallel for private(j,k)
    for (i = 0; i < bs; ++i)
    {
      ...
      {
        for (k = 0; k < ks; ++k)
          n_output_data[k] = (n_input_data[k] > 0) ? n_input_data[k] : weight_data[j] * (exp(n_input_data[k]) - 1);
        n_input_data += ks;
        n_output_data += ks;
      }
    }
  ...
  if (nOutputPlane == 0)
  {
    ...
    TH_TENSOR_APPLY3(real, gradInput, real, gradOutput, real, input,
       if ((*input_data) > 0)
         ...
         *gradInput_data = w * (exp(*input_data)) * (*gradOutput_data);
    );
  }
  else
  {
      ...
      for (j = 0; j < nOutputPlane; ++j)
      {
        ...
        for (k = 0; k < ks; ++k)
        {
          if (n_input_data[k] > 0)
            ...
            n_gradInput_data[k] = n_gradOutput_data[k] * w * (exp(n_input_data[k]));
        }
        n_input_data += ks;
        n_gradInput_data += ks;
          ...
          real scale)
{
  ...
  if (nOutputPlane == 0)
  {
    ...
    TH_TENSOR_APPLY2(real, input, real, gradOutput,
      if ((*input_data) <= 0)
        sum += (exp(*input_data) - 1) * (*gradOutput_data);
    );
  ...
  else
  {
    ...
    for (i = 0; i < bs; ++i)
    {
      ...
      for (j = 0; j < nOutputPlane; ++j)
      {
        ...
        for (k = 0; k < ks; ++k)
          if (n_input_data[k] <= 0)
            sum += n_gradOutput_data[k] * (exp(n_input_data[k]) - 1);
